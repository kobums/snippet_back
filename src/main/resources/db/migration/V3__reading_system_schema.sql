...
-- 1. book_tb 테이블 컬럼 추가
ALTER TABLE book_tb ADD COLUMN b_publisher VARCHAR(100) NOT NULL DEFAULT '' COMMENT '출판사';
ALTER TABLE book_tb ADD COLUMN b_totalpage INT NOT NULL DEFAULT 0 COMMENT '총 페이지 수';
ALTER TABLE book_tb ADD COLUMN b_publicationdate DATE NOT NULL DEFAULT CURRENT_DATE COMMENT '출판일';
...
ALTER TABLE record_tb CHANGE COLUMN s_id r_id BIGINT NOT NULL AUTO_INCREMENT COMMENT '기록 고유 ID';
...
ALTER TABLE record_tb ADD COLUMN r_relatedpage INT NOT NULL DEFAULT 0 COMMENT '관련 페이지 번호';

-- 인덱스 이름 변경
ALTER TABLE record_tb DROP FOREIGN KEY fk_snippet_book;
...
ALTER TABLE record_tb ADD CONSTRAINT fk_record_book FOREIGN KEY (r_book) REFERENCES book_tb (b_id);
